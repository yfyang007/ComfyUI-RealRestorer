"""
Core layer definitions for RealRestorer / Step1X-Edit DiT.
...
def attention(q, k, v, mode="torch", drop_rate=0.0, attn_mask=None, causal=False):
    if mode == "flash" and flash_attn_func is None:
        mode = "torch"

    pre_attn, post_attn = MEMORY_LAYOUT[mode]
    q = pre_attn(q)
    k = pre_attn(k)
    v = pre_attn(v)
...
def scale_add_residual(x, scale, residual):
...
class MLP(nn.Module):
    def __init__(self, in_channels, hidden_channels=None, out_features=None,
                 act_layer=nn.GELU, norm_layer=None, bias=True, drop=0.0,
                 device=None, dtype=None):
        super().__init__()
        out_features = out_features or in_channels
        hidden_channels = hidden_channels or in_channels
        self.fc1 = nn.Linear(in_channels, hidden_channels, bias=bias, device=device, dtype=dtype)
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop)
        self.norm = norm_layer(hidden_channels, device=device, dtype=dtype) if norm_layer is not None else nn.Identity()
        self.fc2 = nn.Linear(hidden_channels, out_features, bias=bias, device=device, dtype=dtype)
...
class TimestepEmbedder(nn.Module):
    def __init__(self, hidden_size, act_layer, frequency_embedding_size=256,
    ...
    def forward(self, t):
        t_freq = self.timestep_embedding(t, self.frequency_embedding_size, self.max_period).type(
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, pe: Tensor) -> Tensor:
        ...
        q, k = self.norm(q, k, v)
        x = attention_after_rope(q, k, v, pe=pe, mode="torch")
        return self.proj(x)
...
class DoubleStreamBlock(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, mlp_ratio: float,
                 qkv_bias: bool = False, mode: str = "torch"):
        super().__init__()
...
class SingleStreamBlock(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, mlp_ratio: float = 4.0,
                 qk_scale: float | None = None, mode: str = "torch"):
        super().__init__()
