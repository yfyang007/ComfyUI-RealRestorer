"""
Step1XEdit DiT -- the denoising transformer backbone for RealRestorer.
...
from .components import Qwen2Connector
from .layers import (
    DoubleStreamBlock,
    EmbedND,
    LastLayer,
...
class Step1XEdit(nn.Module):
    def __init__(self, params: Step1XParams):
        ...
        # Force mode to "torch" to use SDPA (no flash_attn requirement)
        attn_mode = "torch"
        self.double_blocks = nn.ModuleList([
            DoubleStreamBlock(
                self.hidden_size, self.num_heads,
                mlp_ratio=params.mlp_ratio, qkv_bias=params.qkv_bias, mode=attn_mode,
            )
        ...
        self.single_blocks = nn.ModuleList([
            SingleStreamBlock(
                self.hidden_size, self.num_heads,
                mlp_ratio=params.mlp_ratio, mode=attn_mode,
            )
